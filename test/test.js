...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof iterSquaredTriangularSeq, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function throws an error if provided a first argument which is not an object', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			iterSquaredTriangularSeq( value );
		};
	}
});
...
tape( 'the function returns an iterator protocol-compliant object which generates a sequence of squared triangular numbers', function test( t ) {
	var expected;
	var actual;
	var it;
	var i;

	it = iterSquaredTriangularSeq();
	...
	for ( i = 0; i < 1000; i++ ) {
		...
		t.strictEqual( isTriangularNumber( sqrt( actual.value ) ), true, 'returns a squared triangular number' );
...
tape( 'the function supports limiting the number of iterations', function test( t ) {
	...
	it = iterSquaredTriangularSeq( opts );
	t.strictEqual( it.next.length, 0, 'has zero arity' );
...
tape( 'if an environment supports `Symbol.iterator`, the returned iterator is iterable', function test( t ) {
	var iterSquaredTriangularSeq;
	var it1;
	var it2;
	var i;

	iterSquaredTriangularSeq = proxyquire( './../lib/main.js', {
		'@stdlib/symbol-iterator': '__ITERATOR_SYMBOL__'
	});

	it1 = iterSquaredTriangularSeq();
